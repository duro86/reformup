<template>
  <div class="modal fade" id="ComentarioModalAdmin" tabindex="-1" aria-hidden="true" ref="modal">
    <div class="modal-dialog modal-lg modal-dialog-scrollable">
      <div class="modal-content">
        ...
        <div class="modal-body" v-if="loaded">
          ...
          <!-- Profesional -->
          <div class="mb-3" v-if="comentario.profesional">
            <h6 class="fw-semibold mb-1">Profesional</h6>
            <p class="mb-0">
              <strong>{{ comentario.profesional.empresa }}</strong><br>
              <small class="text-muted">
                {{ comentario.profesional.ciudad }}
                <span v-if="comentario.profesional.provincia">
                  - {{ comentario.profesional.provincia }}
                </span>
              </small>
            </p>
          </div>
              ...
              <p class="mb-0">
                <span class="text-warning">
                  <i
                    ...
                    :key="i"
                    class="bi"
                    :class="i <= comentario.puntuacion ? 'bi-star-fill' : 'bi-star'"
                  ></i>
                </span>
...
export default {
  ...
  methods: {
    async openModal(id) {
      ...
      try {
        const resp = await window.axios.get(`/admin/comentarios/${id}`, {
